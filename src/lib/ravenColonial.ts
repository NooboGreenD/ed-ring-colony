...
/** Преобразовать commodities + commanders в ресурсы */
function parseProjectResources(details: any): any[] {
  const commodities: Record<string, number> = details?.commodities || {};
  const commanders: Record<string, Record<string, number>> = details?.commanders || {};

  // Считаем, сколько доставлено по каждому товару
  const delivered: Record<string, number> = {};
  for (const cmdrDeliveries of Object.values(commanders)) {
    for (const [commodity, amount] of Object.entries(cmdrDeliveries as Record<string, number>)) {
      delivered[commodity] = (delivered[commodity] || 0) + amount;
    }
  }

  const resources: any[] = [];
  for (const [key, required] of Object.entries(commodities)) {
    const provided = delivered[key] || 0;
    const remaining = Math.max(0, (required as number) - provided);
    // Преобразуем camelCase/snake_case в читаемое название
    const name = key
      .replace(/([A-Z])/g, ' $1')
  ...
  return resources.sort((a, b) => b.required - a.required);
}

/** Вычислить прогресс проекта */
function calculateProjectProgress(details: any, resources: any[]): number {
  if (details?.complete) return 100;
  if (details?.progress != null) return details.progress;

  if (resources.length > 0) {
    const totalRequired = resources.reduce((s, r) => s + r.required, 0);
    const totalProvided = resources.reduce((s, r) => s + r.provided, 0);
    if (totalRequired > 0) {
      return Math.min(100, Math.round((totalProvided / totalRequired) * 100));
    }
  }

  // Fallback на sumNeed/maxNeed
  if (details?.maxNeed > 0 && details?.sumNeed != null) {
    const delivered = details.maxNeed - details.sumNeed;
    return Math.min(100, Math.round((delivered / details.maxNeed) * 100));
  }
...
export async function fetchRavenSystemV2(name: string): Promise<RavenSystemV2> {
  ...
  for (const proj of activeProjects) {
    ...
    // Новый формат API: commodities + commanders вместо resources
    const resources = parseProjectResources(details);
    const progress = calculateProjectProgress(details, resources);
    const isComplete = details.complete === true || progress >= 100;
