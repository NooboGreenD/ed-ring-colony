...
function RoutePoint({ point, isSelected, onClick }: { point: RouteSystem; isSelected: boolean; onClick: () => void }) {
  ...
  return (
    <group position={[pos.x, pos.y, pos.z]}>
      {/* Свечение (ореол) */}
      <mesh ref={glowRef}>
        <sphereGeometry args={[isSelected ? 7 : 5, 16, 16]} />
        <meshBasicMaterial
          color={color}
          transparent
          opacity={0.10}
      ...
      <mesh ref={meshRef} onClick={onClick} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
        <sphereGeometry args={[isSelected ? 4 : 2.5, 16, 16]} />
      ...
      <Text position={[0, 5, 0]} fontSize={2.5} color="#9ca3af" anchorX="center" anchorY="bottom" visible={hovered || isSelected}>
        {point.system_name}
      </Text>
    </group>
  );
