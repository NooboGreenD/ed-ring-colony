...
// Следит за изменениями данных и форсирует перерисовку Canvas
function DataWatcher({
  ...
  useEffect(() => {
    const hash =
      JSON.stringify(allRouteSystems.map((s) => `${s.system_name}:${s.status}:${s.progress}`)) +
      JSON.stringify(hubs.map((h) => `${h.name}:${h.status}:${h.progress}`));

    if (hash !== prevHashRef.current) {
      prevHashRef.current = hash;
      invalidate();
    }
  }, [allRouteSystems, hubs, invalidate]);
...
export function GalaxyScene({
  ...
  return (
    <>
      ...
      <DataWatcher allRouteSystems={allRouteSystems} hubs={hubs} invalidate={invalidate} />
