...
export default function WikiArticleContent({ article, tags, related }: WikiArticleContentProps) {
  ...
  return (
    <div>
      ...
      {article.image_url && (
        <div style={{ marginBottom: 28, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--line)', position: 'relative' }}>
          <img 
      ...
      <div className="wiki-content" style={{ lineHeight: 1.7, fontSize: 15, color: 'var(--text)' }}>
        <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]} components={{
          img: ({node, ...props}) => (
            <img {...props} style={{maxWidth: '100%', borderRadius: 4, border: '1px solid var(--line)', display: 'block', margin: '16px 0'}} loading="lazy" />
          )
        }}>
          {content}
        </ReactMarkdown>
