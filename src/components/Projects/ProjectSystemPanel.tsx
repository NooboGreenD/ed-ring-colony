"use client";
import { IconStar, IconTarget, IconMapPin, IconUserWorker, IconPackage, IconCheckCircle, IconRefresh, IconChevronUp, IconChevronDown } from "@/components/Icons";
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {system.priority > 1 && (
            <span style={{ fontSize: 11, color: '#e67e22' }}>
              {Array.from({ length: system.priority }).map((_, i) => (
                <IconStar key={i} size={11} color='#e67e22' />
              ))}
            </span>
          )}
          {system.target_date && (
            <span style={{ fontSize: 11, color: '#9ca3af' }}>
              <IconTarget size={11} /> {new Date(system.target_date).toLocaleDateString('ru-RU')}
            </span>
          )}
        ...
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid #323538' }}>
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div style={{ fontSize: 12, color: '#9ca3af' }}>
                  {ravenData.siteName && <span><IconMapPin size={11} /> {ravenData.siteName} · </span>}
                  {ravenData.architectName && <span><IconUserWorker size={11} /> {ravenData.architectName} · </span>}
                  <span><IconPackage size={11} /> {ravenData.projects?.length || 0} проектов</span>
                </div>
          ...
          {canEdit && (
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              ...
              >
                {syncing ? '<IconRefresh size={12} /> Синхронизация...' : '<IconRefresh size={12} /> Синхронизировать RC'}
              </button>
            </div>
          )}
        </div>
