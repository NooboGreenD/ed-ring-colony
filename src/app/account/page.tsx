...
export default function AccountPage() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const res = await fetch(`/api/squadrons/my`);
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={async () => {
                    const res = await fetch('/api/squadrons', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newSquadronName, tag: newSquadronTag || undefined, description: newSquadronDesc || undefined }) });
                    const json = await res.json();
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                ...
                <button onClick={async () => {
                  ...
                  await fetch(`/api/squadrons/${mySquadron.squadron.id}/members`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: user.id }) });
                  setMySquadron(null);
