...
export async function DELETE(req: NextRequest) {
  ...
  // 1. Находим запись и проверяем права
  let findQuery = service.from("friends").select("id, requester_id, addressee_id");
  if (id) {
    findQuery = findQuery.eq("id", id);
  } else {
    findQuery = findQuery.eq("requester_id", requester_id).eq("addressee_id", addressee_id);
  }
  ...
  // 2. Удаляем по найденному id
  const { error } = await service.from("friends").delete().eq("id", existing.id);
