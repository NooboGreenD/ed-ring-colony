...
import SquadronVoiceChat from "@/components/SquadronVoiceChat";

type Tab = "overview" | "pilots" | "ranks" | "projects" | "settings" | "chat" | "voice";
...
export default function SquadronPage() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetch(`/api/squadrons/${id}`);
  ...
  return (
    <div className="card" style={{ width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isCreator && (
            <button onClick={async () => {
              ...
              await fetch(`/api/squadrons/${id}`, { method: "DELETE" });
      ...
      <div className="tabs" style={{ marginBottom: 20 }}>
        ...
        {myMembership && (
          <>
            ...
            <button className={tab === "voice" ? "tab tab-active" : "tab"} onClick={() => setTab("voice")}>Голос</button>
          </>
        )}
      </div>
      ...
      {/* Голосовой чат */}
      {tab === "voice" && myMembership && (
        <SquadronVoiceChat
          squadronId={id}
          userId={user.id}
          isOfficer={isOfficer}
          myName={user.user_metadata?.cmdr_name || user.email || "Пилот"}
        />
      )}
    </div>
  );
...
function SquadronPilotsTab({ squadronId, members, ranks, canManage, userId, onUpdate, setMsg }: any) {
  ...
  const kick = async (user_id: string) => {
    ...
    await fetch(`/api/squadrons/${squadronId}/members`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
...
function SquadronRanksTab({ squadronId, ranks, canManage, onUpdate, setMsg }: any) {
  ...
  const create = async () => {
    ...
    const res = await fetch(`/api/squadrons/${squadronId}/ranks`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function SquadronProjectsTab({ squadronId, projects, canManage, onUpdate, setMsg }: any) {
  ...
  const create = async () => {
    ...
    const res = await fetch(`/api/squadrons/${squadronId}/projects`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
