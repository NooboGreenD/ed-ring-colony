...
кнопка «Обновить сейчас»: по флажкам она делает `git fetch` нужной ветки,
`pg_dump` («с бэкапом БД»), применяет новые `supabase/migrations/*.sql`
(«с миграциями»), пересобирает `web`/`jobs`/`monitor-agent`
(«с тестами» — иначе сборка быстрее) и ждёт ответа `/api/health`. Кнопка
«Применить только миграции» делает то же с базой без пересборки. Прогресс по
стадиям виден админу в панели, а в шапке сайта всем посетителям на это время
