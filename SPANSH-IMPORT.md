...
- **Админка → вкладка «Каталог систем»** (`/admin?tab=galaxy`, только роль
  `admin`): статус каталога, кнопки «Запустить импорт» / «Продолжить» /
  «Остановить», прогресс в процентах и байтах, хвост лога.
- **`POST /api/admin/galaxy`** — то же самое по API (нужна сессия админа);
  `GET` возвращает состояние, `{"action":"cancel"}` останавливает.
...
**Прерывание и продолжение.** Состояние (фаза, счётчики, точка продолжения)
сохраняется в `galaxy_systems_meta` (key `import`), поэтому перезапуск
контейнера не обнуляет прогресс. Дамп — это gzip, а его нельзя
декодировать с середины, поэтому продолжение скачивает файл заново, но
**пропускает уже записанные системы** (точка продолжения — смещение в
распакованном потоке; записи до неё уже в таблице). Повторная запись ~1.3M
строк при этом не выполняется. Записи идемпотентны (upsert по `name_lc`).

**Своё зеркало дампа.** Если сервер не может скачать файл с
`downloads.spansh.co.uk`, задайте `GALAXY_IMPORT_URL` — адрес своего зеркала
(формат тот же: `systems.json.gz`).
...
- `GET /api/galaxy/all-systems` — бинарный файл точек (`edgs-v1`, ETag).
  ...
  данных пока нет;
- `GET|POST /api/admin/galaxy` — статус и управление импортом (сессия
  администратора): `{"action":"start"|"cancel", "fresh?", "truncate?",
  "skip_points?", "url?"}`;
...
- RAM во время импорта: < 1 GB (стриминг, дамп не пишется на диск; облако
  точек в памяти — ~40 МБ);
- время: скачивание ~6 GiB (зависит от линии) + запись ~1.3M строк:
  ~2–5 минут через прямой Postgres, ~20–60 минут через PostgREST;
- диск: не требуется (импорт из приложения ничего не сохраняет на диск;
  CLI-вариант кладёт дамп в `data/spansh/`, это ~6 ГиБ);
