...
1. `curl -s https://edringcolony.ru/api/galaxy/stats | python3 -m json.tool`
  ...
  - `supabase upsert failed: canceling statement due to statement timeout`
    ...
    пополам и повторяет с backoff; строка, которую база не берёт даже по одной
    (текст ошибки называет её: `(имя / id64)`), откладывается, не роняя
    соседние, и дописывается в конце прохода. Ошибка всплывает, только если
    база не приняла строки и тогда — просто продолжите импорт с сохранённой
    точки. Если ошибка повторяется регулярно, задайте веб-процессу
    `SUPABASE_DB_URL` (прямой Postgres вместо PostgREST) или запускайте импорт
    при меньшей загрузке БД;
