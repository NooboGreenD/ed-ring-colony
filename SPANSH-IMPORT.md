...
лимитам, а карта не получит облако в production-образе. Поиск маршрута
берёт куб из каталога только после полного импорта (`systems_count` ≥ 1 000 000
и не `--limit`); пробный импорт не подменяет EDSM.
