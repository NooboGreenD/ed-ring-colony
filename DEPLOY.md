...
- `RUN_TESTS=0` в `.env.production` — пропустить `npm test` внутри образа
  (экстренный режим; по умолчанию проверки обязательны);
- `UPDATE_TIMEOUT_MINUTES` (по умолчанию 90) — предел, после которого
  апдейтер принудительно останавливает обновление; раньше было 45, и холодная
  сборка упиралась ровно в него;
