...
`USE_CACHE=1`, откат — `bash deploy/rebuild-now.sh --rollback`). Для сервера, обновлённого
...
Если сборка упала на проверке TypeScript, сначала исправьте ошибку и
проверьте исходники через `npm run typecheck` там, где установлены зависимости.
`SKIP_TESTS=1` отключает только `npm test`, не проверку типов внутри `next build`.
Повтор ручной сборки с сохранением уже установленных зависимостей:

```bash
# Из корня клона, после получения исправлений:
sudo USE_CACHE=1 bash deploy/rebuild-now.sh
```

Без `USE_CACHE=1` скрипт по-прежнему намеренно пересобирает все слои без кэша.
Кэш не отключает проверки: изменившиеся исходники заново проходят сборку и
проверку типов; контейнеры переключаются только после успешного build.
Стадия `builder` наследует `deps`, поэтому отдельного копирования большого
`node_modules` между стадиями больше нет.

Указанные времена не являются гарантией: HDD, занятый дампом БД или
обслуживанием PostgreSQL, может задерживать даже файловые операции на часы.
Проверяйте `iostat`/`vmstat` и не совмещайте тяжёлый дамп с холодной сборкой.
Предупреждения Turbopack о tracing — не то же самое, что ошибка TypeScript;
исправлять `TS…` отключением проверки типов нельзя.
