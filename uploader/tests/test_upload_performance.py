...
class ApiClientUploadTests(unittest.TestCase):
    ...
    def test_slow_chunk_is_split_and_retried_smaller(self):
        """Пачка, которую сервер не осилил, дробится и уходит частями."""
        attempts = []

        def slow_on_big_chunks(payload):
            rows = payload.get("deliveries", [])
            attempts.append(len(rows))
            return len(rows) > 50  # пачки больше 50 отвечают 500

        client, calls = self._client_with_stub(latency=0.0, fail_predicate=slow_on_big_chunks)
        deliveries = [{"system_name": "Sol", "commodity": "steel", "amount": 1,
                       "delivered_at": "2025-01-01T00:00:00Z", "source_hash": "h%d" % i}
                      for i in range(100)]
        result = client.upload_deliveries(deliveries, "CMDR")
        ...
        self.assertEqual(result["inserted"], 100)
        # Сначала целая пачка, потом две половинки.
        self.assertIn(100, attempts)
