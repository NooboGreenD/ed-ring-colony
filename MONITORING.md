...
3. `compare` — если развёрнутая ревизия уже равна удалённой, обновление
   завершается без пересборки (в панели это «уже актуально», а не ошибка);
   локальные коммиты сверх ветки — остановка, а не «разлить ветку за вас»;
4. `compare 35` — перемотка исходников: незакоммиченные правки на время
   `git merge --ff-only` уносятся в `git stash push -u` (не удаляются) и сразу
   возвращаются `stash pop --index` с явным предупреждением, если вернуть не
   вышло. Спрятано и возвращено ровно вокруг merge — ранний выход «обновлять
   нечего» не оставляет правки в stash;
5. `backup` — `pg_dump -Fc` в `UPDATE_BACKUP_DIR` (или в
   `$UPDATE_STATE_DIR/backups`, если он недоступен) перед любыми миграциями;
...
6. `migrate` — применяются **только** новые `supabase/migrations/*.sql`,
   появившиеся между старой и новой ревизией (`git diff --diff-filter=A`), и
   только те, что ещё не отмечены в `migrations.mark`: повторный запуск ничего
   не накатывает второй раз. Ошибка миграции останавливает обновление **до**
   пересборки и переключения кода — сайт остаётся на рабочей версии;
