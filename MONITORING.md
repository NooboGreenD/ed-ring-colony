...
7. `build` + `switch` — `docker compose --profile monitoring up -d --build
   ...
   **«Перезапустить агент»** — на случай, если автоперезапуск отключён;
...
Лимита времени на обновление по умолчанию НЕТ (старые 45 минут отменены — в
них холодная сборка не влезала): апдейтер остановит сборку только если в
`.env.production` явно задан `UPDATE_TIMEOUT_MINUTES=<число>`; строка со
значением `45` в файле — удалите её. Быстрый режим — флажок «с тестами»
в панели или `RUN_TESTS=0` (пропустить тесты в образе).
